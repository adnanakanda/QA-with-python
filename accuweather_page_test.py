...
class Test_accuweather_test(Test_Base):
    ...
    def test_webui_test(self):
        ...
        result_container_element = self.wait.until(E_C.visibility_of_element_located(self.result_container))
        assert result_container_element.is_displayed(), "Search results container is not displayed"

        # Click on the first search result.
        first_search_result_element = self.wait.until(E_C.visibility_of_element_located(self.first_search_result))
        first_search_result_element.click()

        # Check the weather City weather page header contains city name from the search.
        city_weather_header = self.wait.until(E_C.visibility_of_element_located(self.location_header))
